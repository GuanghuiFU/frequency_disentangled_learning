...
class Unet3D_disentangle_late_fuse(BaseModel):
    """
    Implementations based on the Unet3D paper: https://arxiv.org/abs/1606.06650
    """

    def __init__(self, in_channels, n_classes, base_n_filter=8):
        super(Unet3D_disentangle_late_fuse, self).__init__()
    ...
    def forward(self, x_low_i, x_high_i):
        ...
        output_high_f = fftn(out, dim=tuple(range(2, out.ndim)))
        output_low_high_f = merge_low_high(output_low_f, output_high_f)
        # print('forward output_low_high_f.size():', output_low_high_f.size())
        out_low_high = ifftn(output_low_high_f, dim=tuple(range(2, output_low_high_f.ndim))).type(torch.float32)
        out_low_high = torch.cat([out_low_high, context_1], dim=1)
        # print('out_low_high conv3dl2 & 2:', out_low_high.shape)
        out_low_high = self.conv_norm_lrelu_l4(out_low_high)
        ds3 = out_low_high
        out_low_high = self.conv3d_l4(out_low_high)
        # print('out_low_high conv3dl3:', out_low_high.shape)
        out_low_high_pred = out_low_high
        ds3_1x1_conv = self.ds3_1x1_conv3d(ds3)
        out_low_high = out_low_high_pred + ds3_1x1_conv
...
if __name__ == '__main__':
    unet_fftn = Unet3D_disentangle_late_fuse(in_channels=2, n_classes=2)
    unet_fftn.test()
