...
class BaseModel(nn.Module, ABC):
    ...
    def save_checkpoint(self,
        ...
        if name is None:
            name = "{}_{}_epoch.pth".format(os.path.basename(directory),  # netD or netG
                'last')

        torch.save(ckpt_dict, os.path.join(directory, name))
        if self.best_loss > loss:
            self.best_loss = loss
            name = "{}_{}e_BEST.pth".format(os.path.basename(directory), epoch)
            torch.save(ckpt_dict, os.path.join(directory, name))
    ...
    # [Multi-input]
    #def inference(self, input_tensor_low, input_tensor_high):
    def inference(self, input_tensor_low):
        self.eval()
        with torch.no_grad():
            # [Multi-input]
            #output = self.forward(input_tensor_low, input_tensor_high)
            output = self.forward(input_tensor_low)
