...
def compute_per_channel_dice(input, target, target_expand, epsilon=1e-6, weight=None):
    """
...
def compute_per_channel_dice_eval(input, target, epsilon=1e-6):
    ...
    denominator = (input * input).sum(-1) + (target * target).sum(-1)
    return 2 * (intersect / denominator.clamp(min=epsilon)).numpy()
