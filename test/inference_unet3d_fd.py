...
from lib.visual3D_temp import visualize_3D_no_overlap_fftn
...
def mask_tensor(input_tensor, beta=0.1):
    # print('mask_tensor input_tensor.size():',input_tensor.size())
    ...
    # print('width, mask_width: ', width, mask_width)
    mask_star_x = int(width / 2 - mask_width / 2)
    mask_star_y = mask_star_x
    mask_end_x = int(width / 2 + mask_width / 2)
    mask_end_y = mask_end_x
    # print('mask_star_x:mask_end_x, mask_star_y:mask_end_y:', mask_star_x, mask_end_x, mask_star_y, mask_end_y)
...
def main():
    ...
    # For [Red Nucleus]
    #model.restore_checkpoint('../saved_models/isbi_unet3d_fftshift_new_'+args.fft_ratio+'/'+trained_model_base_path+'/' + trained_model_name + '.pth')
    # For [Red Nucleus]
    model.restore_checkpoint('../saved_models/'+trained_model_base_path+'/' + trained_model_name + '.pth')
    if args.cuda:
        model = model.cuda()
    ...
    args.save = os.path.join('../inference_checkpoints/test/fourier/', cross_validation_index, trained_model_name + '/')
    ...
    for select_scan in scan_list_npy:
        ...
        input_dim = tuple(range(2, mri_tensor.ndim))
        input_f = fftn(mri_tensor, dim=input_dim)
        ...
        #input_low_input = input_low_i
        #input_low_input = input_low_target_list
        input_low_input = input_low_i
        input_low_input, input_high_i = input_low_input.cuda(), input_high_i.cuda()
        seg_tensor = path2tensor(seg_path, 'seg')
        summary, dice, precision, recall, mver, maver, pearsonsr, f1, topo_2d_err, topo_3d_err = visualize_3D_no_overlap_fftn(args=args, filename=select_scan, full_volume_low_list=input_low_input, full_volume_high=input_high_i, seg_map=seg_tensor, num_classes=args.classes, model=model,affine=nib.load(mri_path).affine)
...
def get_arguments():
    ...
    parser.add_argument('--trained_model_base_path', type=str)
    parser.add_argument('--modalities', type=str, default="heart")
    parser.add_argument('--regions', type=str, default="2")
